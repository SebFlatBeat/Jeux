...
public class Mastermind extends Game{
    ...
    public void challengerMode() {
        ...
        if (choixJoueur == 1) {
            ...
            LOGGER.info("Souhaites-tu revenir au menu du jeu com.enedis.sebastien.Jeux.Mastermind?");
            System.out.println("1 - Oui");
            System.out.println("2 - Non");
            do {
                try {
    ...
    super.defenseMode();
        ...
        while (!Arrays.equals(tabReponse, getJoueurTabMystere()) && getNombreEssais() < getNbMaxEssais()) {
            ...
            for (int trial = 0; trial < joueurNbMystere ; trial++) {
                Knuth.Fonction<Integer, List<Integer>> setOfN = KnuthAlgo.setOfN(joueurNbMystere);
                for (int i = 0; i < getJoueurTabMystere().length; i++) setOfN.appelle(i);
                for (int s : setOfN.appelle(getJoueurTabMystere().length-1)) bin[s]++;
            }

            int [] compare = new int[joueurNbMystere];

            for (int i = 0;i<getJoueurTabMystere().length;i++) {
                boolean rechercheBienPlace = Arrays.equals(bin, compare);
